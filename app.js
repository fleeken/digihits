...
async function animateCardDeal(card, starter = null, beforeClose = null) {
  if (matchMedia("(prefers-reduced-motion: reduce)").matches) { beforeClose?.(); return; }
  skipCardDeal = false;
...
async function enterNewCardGuess() {
  const deal = pendingTimelineDeal;
  pendingTimelineDeal = null;
  if (deal) {
    showView("timeline");
    await animateCardDeal(deal.card, deal.starter, () => showView("guess"));
  } else showView("guess");
  startCurrentTrack();
}
...
if ("serviceWorker" in navigator) navigator.serviceWorker.register("sw.js?v=5.89", { updateViaCache: "none" }).then((registration) => registration.update()).catch(() => {});

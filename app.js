...
async function persistAvatar() { const token = supabaseAuth.session()?.access_token, a = avatarChoice(state.avatar), traits = avatarRig(state.avatar); if (!token) return; await supabaseAuth.dataRequest("rpc/digihits_set_avatar", { chosen_genre: a.genre, chosen_variant: a.variant, chosen_traits: traits }, "POST").catch(() => supabaseAuth.dataRequest("rpc/digihits_set_avatar", { chosen_genre: a.genre, chosen_variant: a.variant }, "POST")).catch(() => {}); }
...
let lastCareerSync = "";
function persistCareer() { const points = Number(state.stats?.wins || 0) * 3 - Number(state.stats?.walkoverLeaves || 0) + Number(state.stats?.achievementXp || 0), key = String(points); if (key === lastCareerSync || !supabaseAuth.session()?.access_token) return; lastCareerSync = key; supabaseAuth.dataRequest("rpc/digihits_set_career", { career_points_input: points }, "POST").catch(() => { lastCareerSync = ""; }); }
function renderFriends() {
  ...
  friends.innerHTML = `<h3 class="friend-section-title">Vänskapslista</h3>${state.friends.length ? state.friends.map((friend) => { const avatar = avatarChoice(friend); return `<article class="friend-row friend-profile" data-avatar-genre="${avatar.genre}" data-avatar-variant="${avatar.variant}"><strong>${escapeHtml(friend.display_name)}</strong><div class="friend-actions friend-main-actions"><button class="button button-green" data-create-friend-match="${friend.friend_id}" type="button">SKAPA NY MATCH MOT</button><button class="button button-leave" data-remove-friend="${friend.friend_id}" data-friend-name="${escapeHtml(friend.display_name)}" type="button">TA BORT VÄN</button></div></article>`; }).join("") : `<p class="friend-empty">Du har inga vänner ännu.</p>`}${state.friendNotifications.map((notice) => `<article class="friend-row friend-request-declined"><strong>${escapeHtml(notice.display_name)} tog bort dig som vän.</strong><button class="button button-secondary" data-dismiss-friend-notice="${notice.notice_id}" type="button">OK</button></article>`).join("")}`;
  friends.querySelectorAll(".friend-row.friend-profile").forEach((row) => { const name = row.querySelector(":scope > strong"), actions = row.querySelector(".friend-main-actions"); if (!name || !actions) return; const create = actions.querySelector("[data-create-friend-match]"), remove = actions.querySelector("[data-remove-friend]"), friend = state.friends.find((item) => String(item.friend_id) === String(create?.dataset.createFriendMatch)), avatar = document.createElement("span"), identity = document.createElement("div"); avatar.className = "mini-avatar friend-avatar avatar-rig-mini"; avatar.innerHTML = avatarRigSvg(friend?.avatar_traits || friend || {}, true); identity.className = "friend-identity"; name.before(identity); identity.append(avatar, name); const career = document.createElement("button"); career.type = "button"; career.className = "button friend-career-button"; career.dataset.friendCareer = friend?.friend_id || ""; career.textContent = state.friendCareerOpen[friend?.friend_id] ? "DÖLJ MUSIKKARRIÄR" : "VISA MUSIKKARRIÄR"; remove ? actions.insertBefore(career, remove) : actions.append(career); if (state.friendCareerOpen[friend?.friend_id]) { const points = Number(friend?.career_points || 0), level = careerLevel(points), card = document.createElement("section"); card.className = "friend-career-card level-panel"; card.innerHTML = `<div class="level-head"><div><small>ONLINE-NIVÅ</small><b>${level}</b></div></div><div class="level-progress"><i style="width:${careerProgress(points)}%"></i><strong>ONLINEPOÄNG: ${points}</strong></div><small class="level-next">${level === "Digihits-legendar" ? "HÖGSTA NIVÅN" : "FORTSÄTT SPELA FÖR NÄSTA NIVÅ"}</small><section class="achievement-list"><h3>UTMÄRKELSER</h3><p>${Number(friend?.career_achievement_count || 0)} UPPNÅDDA UTMÄRKELSER</p></section>`; row.append(card); } });
}
document.addEventListener("click", (event) => { const button = event.target.closest("[data-friend-career]"); if (!button) return; state.friendCareerOpen[button.dataset.friendCareer] = !state.friendCareerOpen[button.dataset.friendCareer]; save(); renderFriends(); });
async function syncFriends() {
  ...
  supabaseAuth.dataRequest("rpc/digihits_my_avatar", {}, "POST").then((rows) => { const saved = rows?.[0]; if (!saved) return; state.avatar = { ...state.avatar, genre: saved.avatar_genre, variant: Number(saved.avatar_variant), traits: saved.avatar_traits || state.avatar?.traits }; save(); renderAvatar(); }).catch(() => {});
}
...
function avatarRigSvg(raw, mini = false) { const a = avatarRig(raw), faceRow = a.skin === "Ljus" ? 0 : a.skin === "Mellan" ? 1 : 2, faceCol = a.presentation === "Maskulin" ? 1 : 0, face = `--face-x:${faceCol * 100}%;--face-y:${faceRow * 50}%`, hairCell = { Kort: 1, Vågor: 2, Lockar: 3, Långt: 2, Buzz: 1, Flätor: 3 }[a.hair] ?? 1, outfitCell = { Pop: 4, Street: 5, Rock: 6, EDM: 4, Indie: 5, Country: 6 }[a.outfit] ?? 4, accessoryCell = a.accessory === "Hörlurar" || a.accessory === "AirPods" ? 7 : a.accessory === "Glasögon" || a.accessory === "Öronringar" ? 8 : -1, cellStyle = (cell) => `--atlas-x:${(cell % 3) * 50}%;--atlas-y:${Math.floor(cell / 3) * 50}%`, tint = { Mörk: "none", Brun: "sepia(.16)", Blond: "sepia(.55) saturate(.7) brightness(1.15)", Röd: "sepia(.35) saturate(1.7) hue-rotate(325deg)", Lila: "sepia(.35) saturate(1.7) hue-rotate(235deg)" }[a.hairColor] || "none"; return `<div class="avatar-rig-art${mini ? " avatar-rig-art-mini" : ""}"><i class="avatar-atlas-layer avatar-atlas-face" style="${face}"></i><i class="avatar-atlas-layer avatar-atlas-part avatar-atlas-outfit" style="${cellStyle(outfitCell)}"></i><i class="avatar-atlas-layer avatar-atlas-part avatar-atlas-hair" style="${cellStyle(hairCell)};filter:${tint}"></i>${accessoryCell >= 0 ? `<i class="avatar-atlas-layer avatar-atlas-part avatar-atlas-accessory" style="${cellStyle(accessoryCell)}"></i>` : ""}</div>`; }
$(".brand small").textContent = "v5.38";
render();

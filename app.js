...
async function persistAvatar() { const token = supabaseAuth.session()?.access_token, a = avatarChoice(state.avatar); if (!token) return; await supabaseAuth.dataRequest("rpc/digihits_set_avatar", { chosen_genre: a.genre, chosen_variant: a.variant }, "POST").catch(() => {}); }
...
async function syncFriends() {
  ...
  supabaseAuth.dataRequest("rpc/digihits_my_avatar", {}, "POST").then((rows) => { const saved = rows?.[0]; if (!saved) return; state.avatar = { ...state.avatar, genre: saved.avatar_genre, variant: Number(saved.avatar_variant) }; save(); renderAvatar(); }).catch(() => {});
}

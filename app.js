...
function render() {
  ...
  const renderCard = (match) => {
    ...
    return `<article class="match ${room ? "room" : solo ? "solo" : match.status}">${solo ? "" : room ? `<div class="match-top"><strong>Samma fysiska rum</strong></div>` : `<div class="match-status match-status-top">● ${status}</div>${unread ? `<button class="match-chat-alert" data-open-chat="${match.code}" title="Nya chattmeddelanden" aria-label="Öppna chatt" type="button">✉<b>${unread}</b></button>` : ""}<button class="match-icon delete-icon match-delete-top" data-delete-match="${match.code}" title="Lämna match" aria-label="Lämna match" type="button">🗑</button>`}${solo ? `<div class="match-top"><strong>Solomatch</strong></div><div class="solo-card-stats"><div><strong>${soloScore.correct}/10</strong><small>RÄTT PLACERADE</small></div><div><strong>${soloScore.mistakes}</strong><small>FELPLACERADE</small></div><div><strong>${match.round || 1}</strong><small>${(match.round || 1) === 1 ? "OMGÅNG" : "OMGÅNGAR"}</small></div></div>` : `<div class="match-player-list">${playerRows}</div>`}<div class="match-footer"><button class="match-open" data-open-match="${match.code}" type="button">${label}</button>${solo || room ? `<div class="match-card-actions"><button class="match-icon delete-icon" data-delete-match="${match.code}" title="Lämna match" aria-label="Lämna match" type="button">🗑</button></div>` : ""}</div>${solo || room ? "" : `<div class="match-code match-code-bottom">MATCHKOD <strong>${match.code}</strong></div>`}</article>`;
  };

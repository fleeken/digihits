...
let viewingLatestRound = false, viewingHistoryResult = false, returnToFinalResult = false, historyResultEntry = null, historyResultRounds = 0;
const latestRounds = {};
...
function renderFriends() {
  ...
  sentMatches.innerHTML = state.sentMatchInvites.length ? `<h3 class="friend-section-title">Skickade matchförfrågningar</h3>${state.sentMatchInvites.map((invite) => `<article class="friend-row ${invite.status === "accepted" ? "friend-request-accepted" : "friend-request-declined"}"><strong>Matchförfrågan ${invite.status === "accepted" ? "accepterad" : "avvisad"} av ${escapeHtml(invite.recipient_name || "spelaren")} · MATCHKOD ${escapeHtml(invite.match_code)}.</strong><button class="button button-secondary" data-dismiss-sent-match-invite="${invite.invite_id}" type="button">OK</button></article>`).join("")}` : "";
  ...
  friends.innerHTML = `<h3 class="friend-section-title">Vänskapslista</h3>${state.friends.length ? state.friends.map((friend) => { const unread = Number(state.friendChatUnread[friend.friend_id] || 0), name = String(friend.display_name || "").toLocaleLowerCase("sv-SE"), played = state.history.filter((match) => match.mode === "online" && String(match.opponentName || String(match.title || "").split(", ").at(-1)).toLocaleLowerCase("sv-SE") === name), wins = played.filter((match) => /VANN/i.test(match.leaveReason || "")).length, losses = played.filter((match) => /FÖRLORADE/i.test(match.leaveReason || "")).length, walkovers = played.filter((match) => /WALK/i.test(match.leaveReason || "")).length; return `<article class="friend-row"><strong>${escapeHtml(friend.display_name)}</strong><div class="friend-stats"><div><b>${wins}</b><small>VINSTER MOT</small></div><div><b>${losses}</b><small>FÖRLUSTER MOT</small></div><div><b>${walkovers}</b><small>WALK OVER</small></div><div><b>${played.length}</b><small>SPELADE MATCHER</small></div></div><div class="friend-actions friend-main-actions"><button class="button button-primary" data-open-friend-chat="${friend.friend_id}" type="button">VISA CHATT${unread ? `<span class="chat-badge">${unread}</span>` : ""}</button><button class="button button-green" data-create-friend-match="${friend.friend_id}" type="button">SKAPA MATCH MOT</button><button class="button button-leave" data-remove-friend="${friend.friend_id}" data-friend-name="${escapeHtml(friend.display_name)}" type="button">TA BORT VÄN</button></div></article>`; }).join("") : `<p class="friend-empty">Du har inga vänner ännu.</p>`}`;
}
async function syncFriends() {
  if (!supabaseAuth.session()?.access_token) return;
  const [friends, requests, invites, unreads, sentRequests, sentMatchInvites] = await Promise.all([supabaseAuth.dataRequest("rpc/digihits_my_friends", {}, "POST"), supabaseAuth.dataRequest("rpc/digihits_my_friend_requests", {}, "POST"), supabaseAuth.dataRequest("rpc/digihits_my_match_invites", {}, "POST"), supabaseAuth.dataRequest("rpc/digihits_my_friend_unreads", {}, "POST"), supabaseAuth.dataRequest("rpc/digihits_my_sent_friend_requests", {}, "POST"), supabaseAuth.dataRequest("rpc/digihits_my_sent_match_invites", {}, "POST")]);
  state.friends = friends || []; state.friendRequests = requests || []; state.friendInvites = invites || []; state.friendChatUnread = Object.fromEntries((unreads || []).map((item) => [item.friend_id, Number(item.unread_count)])); state.sentFriendRequests = sentRequests || []; state.sentMatchInvites = sentMatchInvites || []; save(); renderFriends();
}
...
function showLatestRound(round) {
  ...
  if (playedCard) { $("#result-song").textContent = `${playedCard.artist} – ${playedCard.title} (${playedCard.year})`; renderGuessChecks(playedCard, round.guess || {}); } const solo = Boolean(state.matches.find((match) => match.code === state.activeMatchCode)?.solo); const attempts = Number(round.rounds || 0) || (viewingHistoryResult ? historyResultRounds : 0) || state.matches.find((match) => match.code === state.activeMatchCode)?.round || 0, correctCards = Math.max((round.timeline || round.cards || []).filter((card) => !/FEL ?PLACERAT/.test(card.status)).length, Number(round.score?.correct) || 0), mistakes = Math.max(0, Number(round.score?.mistakes) || Math.max(0, attempts - Math.max(0, correctCards - 1))); $(".result-match-code").hidden = solo; $("#result-code-label").textContent = solo ? "FELPLACERADE KORT" : "MATCHKOD"; $("#result-code").textContent = solo ? String(mistakes) : state.activeMatchCode || "------"; $("#result-code").classList.toggle("solo-mistake-count", solo); $("#solo-result-score").hidden = !(solo || viewingHistoryResult); $("#solo-result-score").innerHTML = soloResultStats(correctCards, mistakes, attempts);
...
async function joinOnlineMatch(matchCode) {
  ...
  await supabaseAuth.dataRequest(`online_matches?id=eq.${match.id}`, { status: "active", ...(match.status === "waiting" ? { phase: "turn_ready", current_user_id: players[0]?.user_id || user.id, turn_started_at: new Date().toISOString(), turn_reminder_sent_at: null, turn_notice: null } : {}), deck, used_track_ids: [...(match.used_track_ids || []), starter.id], updated_at: new Date().toISOString() }, "PATCH");
  rememberTrack(starter); state.changeTrackCards = 0; save(); await syncMatches(); openMatch(matchCode);
}
...
document.addEventListener("click", (event) => { const button = event.target.closest(".final-player-round"); if (button) { returnToFinalResult = true; showLatestRound(latestRounds[button.dataset.playerRound]); } });
...
if ("serviceWorker" in navigator) navigator.serviceWorker.register("sw.js?v=3.22").catch(() => {});
render();

...
document.addEventListener("click", (event) => { if (!event.target.closest("#add-room-player")) return; const holder = $("#room-player-inputs"); if (holder.children.length < 8) { holder.insertAdjacentHTML("beforeend", roomPlayerRow(holder.children.length + 1)); renumberRoomPlayers(); } });

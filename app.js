...
document.addEventListener("click", (event) => {
  const style = event.target.closest("[data-avatar-style]"), variant = event.target.closest("[data-avatar-variant]"), random = event.target.closest("[data-avatar-style-random]");
  if (!style && !variant && !random) return;
  state.avatar ||= {};

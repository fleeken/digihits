...
async function animateCardDeal(card, starter = null) {
  ...
  const fly = async (flyingCard, secret, target, duration) => {
    ...
    const startX = deckBounds.left + deckBounds.width / 2 - targetBounds.width / 2, startY = deckBounds.top + deckBounds.height * .55 - targetBounds.height, exitX = startX, exitY = startY - targetBounds.height * .7;
    ...
    const frames = points.map((point, index) => { const t = point.t ?? (index ? 0 : 0), scale = index < 2 ? 1 : 1 + Math.sin(Math.PI * t) * .025, rotation = index < 2 ? -4 + index * 2 : Math.sin(Math.PI * t * 2) * 1.8; return { offset: offsets[index], transform: `translate3d(${point.x}px,${point.y}px,0) rotate(${rotation}deg) scale(${scale})`, clipPath: index === 0 ? "inset(72% 0 0 0 round 14px)" : "inset(0 0 0 0 round 14px)", opacity: 1 }; });

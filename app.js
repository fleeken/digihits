...
async function syncFriends() {
  ...
  state.friends = friends || []; state.friendRequests = requests || []; state.friendInvites = invites || []; state.friendChatUnread = Object.fromEntries((unreads || []).map((item) => [item.friend_id, Number(item.unread_count)])); state.sentFriendRequests = sentRequests || []; state.sentMatchInvites = sentMatchInvites || []; state.friendNotifications = notifications || []; state.blocks = []; save(); renderFriends();
}
...
function render() {
  ...
  const renderCard = (match) => {
    ...
    const playerRows = matchPlayers.map((player) => { const correct = Math.max(1, Number(player.last_round?.score?.correct) || (player.locked_timeline || []).length || 0); return `<div><strong>${escapeHtml(player.display_name)}</strong><b>${correct}/10</b></div>`; }).join("");
    const lock = match.locked ? "🔒" : "🔓";
    const lockLabel = match.locked ? "Match låst" : "Match olåst";
    return `<article class="match ${solo ? "solo" : match.status}">${solo ? "" : `<button class="match-lock-top ${match.locked ? "is-locked" : "is-unlocked"}" title="${lockLabel}" aria-label="${lockLabel}" type="button">${lock}</button>`}${solo ? `<div class="match-top"><strong>Solomatch</strong></div><div class="solo-card-stats"><div><strong>${soloScore.correct}/10</strong><small>RÄTT PLACERADE</small></div><div><strong>${soloScore.mistakes}</strong><small>FELPLACERADE</small></div><div><strong>${match.round || 1}</strong><small>${(match.round || 1) === 1 ? "OMGÅNG" : "OMGÅNGAR"}</small></div></div>` : `<div class="match-player-list">${playerRows}</div><div class="match-card-metrics"><div><strong>${match.round || 1}</strong><small>OMGÅNG</small></div><div><strong>${matchPlayers.length}/8</strong><small>SPELARE</small></div></div><div class="match-status">● ${status}</div><div class="match-code">MATCHKOD &nbsp; <strong>${match.code}</strong></div>`}<div class="match-footer"><button class="match-open" data-open-match="${match.code}" type="button">● ${label}</button><div class="match-card-actions"><button class="match-icon delete-icon" data-delete-match="${match.code}" title="Lämna match" aria-label="Lämna match" type="button">🗑</button></div></div></article>`;
  };
...
function openMatch(matchCode) {
  ...
  if (!roundLoading) $("#next-round").textContent = pendingRound ? (soloMatch ? "ÅTERUPPTA MATCH" : "ÅTERUPPTA OMGÅNG") : "STARTA NÄSTA OMGÅNG";
...
window.resumeDigihitsRound = async () => { const button = $("#next-round"); if (button.disabled) return; if (!supabaseAuth.spotify()) { dialog("Du måste ansluta till ett Spotify Premium-konto.", () => supabaseAuth.connectSpotify().catch((error) => alert(error.message)), false, "ANSLUT KONTO"); return; } roundLoading = true; button.disabled = true; const label = button.textContent; button.textContent = "LADDAR OMGÅNG…"; try { const pending = state.pendingResult; if (pending?.matchCode === state.activeMatchCode) { currentPlacementCorrect = pending.correct !== false; resultIsLocked = true; renderRoundResult(currentPlacementCorrect, pending.card, pending.snapshot); showView("result"); return; } await syncMatches(); button.textContent = "LADDAR OMGÅNG…"; const match = state.matches.find((item) => item.code === state.activeMatchCode); if (!match || match.status !== "active") throw new Error("Omgången kan inte återupptas just nu."); await restoreRoundUnlocked(); const existingCard = Boolean(state.currentCard); if (existingCard) { showView(state.roundResumeViews[state.activeMatchCode] || "guess"); pausedForNavigation = true; resumeRoundTrack(); return; } state.roundUnlocked = []; save(); await markRoundStarted(); await dealCard(); resetTurnInput(); showView("guess"); startCurrentTrack(); } catch (error) { alert(error.message); } finally { roundLoading = false; button.disabled = false; button.textContent = label; } };

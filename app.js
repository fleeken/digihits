...
function avatarSvg(a, mini = false) { const skin = { Ljus: "#f6c9a7", Mellan: "#c97d50", Mörk: "#70432d" }[a.skin], iris = { Brun: "#61391f", Blå: "#4eb9dd", Grön: "#5fb77a", Grå: "#92a0b4" }[a.eyes], hair = { Mörk: "#201b23", Brun: "#663b2b", Blond: "#d8a14a", Röd: "#b75243", Lila: "#7e51be" }[a.hairColor], outfit = { Street: "#274e7e", Rock: "#25242d", Pop: "#c75c98", EDM: "#5948aa", Indie: "#4f745b", Country: "#9a663a" }[a.outfit]; const hairPath = { Buzz: "M44 57Q47 21 100 19Q153 21 156 57Q128 41 100 42Q72 41 44 57", Vågor: "M38 61Q42 18 73 29Q94 7 113 30Q142 13 164 55Q145 42 129 51Q109 35 96 53Q73 36 55 57Z", Lockar: "M40 64Q33 40 54 32Q60 12 78 29Q91 9 105 28Q122 9 132 31Q153 16 163 43Q170 56 158 65Z", Långt: "M39 64Q37 20 100 19Q163 20 161 64L151 130L135 111L65 111L49 130Z", Mohawk: "M69 59L82 18L100 5L118 18L131 59Z", Flätor: "M43 62Q38 21 100 19Q162 21 157 62L151 127L139 119L130 140L118 115L108 137L96 114L84 137L73 115L61 140L52 119L48 127Z" }[a.hair]; const face = a.presentation === "Maskulin" ? "M52 56Q44 82 53 126Q65 161 100 166Q135 161 147 126Q156 82 148 56Q128 36 100 36Q72 36 52 56" : a.presentation === "Feminin" ? "M54 54Q43 86 57 128Q72 160 100 164Q128 160 143 128Q157 86 146 54Q127 35 100 35Q73 35 54 54" : "M51 55Q43 83 55 127Q69 161 100 165Q131 161 145 127Q157 83 149 55Q127 35 100 35Q73 35 51 55"; const eyeY = a.eyeShape === "Glada" ? 89 : 84, eyeRx = a.eyeShape === "Skarpa" ? 13 : 16, lashes = a.presentation === "Feminin" ? "stroke-width='3'" : "stroke-width='2'"; const accessory = a.accessory === "Hörlurar" ? "<path d='M43 92Q43 42 100 42Q157 42 157 92' fill='none' stroke='#24233a' stroke-width='13'/><rect x='30' y='88' width='23' height='43' rx='10' fill='#5bcff0'/><rect x='147' y='88' width='23' height='43' rx='10' fill='#5bcff0'/>" : a.accessory === "AirPods" ? "<path d='M48 91v25M152 91v25' stroke='#f4f8ff' stroke-width='7' stroke-linecap='round'/><circle cx='48' cy='89' r='6' fill='#f4f8ff'/><circle cx='152' cy='89' r='6' fill='#f4f8ff'/>" : a.accessory === "Glasögon" ? "<circle cx='73' cy='85' r='19' fill='none' stroke='#f1c557' stroke-width='4'/><circle cx='127' cy='85' r='19' fill='none' stroke='#f1c557' stroke-width='4'/><path d='M92 85h16' stroke='#f1c557' stroke-width='4'/>" : a.accessory === "Öronringar" ? "<circle cx='43' cy='113' r='7' fill='none' stroke='#f1c557' stroke-width='3'/><circle cx='157' cy='113' r='7' fill='none' stroke='#f1c557' stroke-width='3'/>" : a.accessory === "Kedja" ? "<path d='M70 178Q100 202 130 178' fill='none' stroke='#f1c557' stroke-width='5'/>" : ""; const hat = a.hat === "Keps" ? "<path d='M43 60Q47 24 101 23Q151 25 157 57L132 55Q106 47 77 56Z' fill='#222633'/><path d='M78 54Q112 51 152 64Q112 67 76 61Z' fill='#3d4558'/>" : a.hat === "Beanie" ? "<path d='M43 60Q46 19 100 19Q154 19 157 60L147 69H53Z' fill='#7e51be'/><path d='M49 58H151V72H49Z' fill='#a36fe5'/>" : ""; return `<svg class="avatar2d-svg${mini ? " mini-svg" : ""}" viewBox="0 0 200 220" aria-hidden="true"><rect width="200" height="220" rx="20" fill="#101a2d"/><path d="M20 220Q26 167 67 157H133Q174 167 180 220" fill="${outfit}"/><path d="M75 152H125V184H75Z" fill="${skin}"/><path d="${face}" fill="${skin}"/><path d="${hairPath}" fill="${hair}"/><ellipse cx="72" cy="${eyeY}" rx="${eyeRx}" ry="9" fill="#fff"/><ellipse cx="128" cy="${eyeY}" rx="${eyeRx}" ry="9" fill="#fff"/><circle cx="72" cy="${eyeY}" r="5" fill="${iris}"/><circle cx="128" cy="${eyeY}" r="5" fill="${iris}"/><path d="M57 ${eyeY-8}Q72 ${eyeY-17} 87 ${eyeY-8}M113 ${eyeY-8}Q128 ${eyeY-17} 143 ${eyeY-8}" fill="none" stroke="#2a1c1a" ${lashes}/><path d="M91 119Q100 126 109 119" fill="none" stroke="#803f42" stroke-width="3" stroke-linecap="round"/>${hat}${accessory}</svg>`; }
function renderAvatarRig() { const panel = $("#avatar-panel"); if (!panel) return; const a = avatarV2(); const accountAvatar = $("#change-avatar"); if (accountAvatar) { accountAvatar.className = "mini-avatar account-avatar"; accountAvatar.innerHTML = avatarSvg(a, true); } panel.innerHTML = `<h3>MIN ARTIST-AVATAR</h3><div class="avatar-v2-layout"><div class="avatar-v2-preview">${avatarSvg(a)}</div><div class="avatar-v2-controls"><p>Välj en genre som grund — ändra sedan precis de delar du vill.</p><div class="avatar-v2-genres">${Object.keys(avatarGenrePresets).map((genre) => `<button type="button" class="${a.genre === genre ? "is-selected" : ""}" data-avatar-v2-genre="${genre}">${genre.toUpperCase()}</button>`).join("")}</div><div class="avatar-v2-grid">${Object.entries(avatarV2Options).map(([part, values]) => `<label>${avatarV2Labels[part]}<select data-avatar-v2="${part}">${values.map((value) => `<option${a[part] === value ? " selected" : ""}>${value}</option>`).join("")}</select></label>`).join("")}</div><button class="button button-purple" type="button" data-avatar-v2-random>SLUMPA HELA AVATAREN</button></div></div>`; }
...
$("#avatar-back")?.addEventListener("click", () => showView("home"));
...
$("#change-track-area").addEventListener("click", async (event) => {
  ...
  dialog("Är du säker på att du vill använda ett av dina byt-låt-kort?", async () => { try { await updateSwapCards(-1); state.swapUsedThisRound = true; save(); await dealCard(); await settlePendingSwapAward(); } catch (error) { alert(error.message); return; } resetTurnInput(); showView("guess"); startCurrentTrack(); }, false, "ANVÄND BYT-LÅT-KORT");
});

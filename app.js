...
document.addEventListener("click", (event) => { if (!event.target.closest("#add-room-player")) return; const holder = $("#room-player-inputs"); if (holder.children.length < 8) { holder.insertAdjacentHTML("beforeend", roomPlayerRow(holder.children.length + 1)); renumberRoomPlayers(); clearRoomPlayerError(); } });
document.addEventListener("click", (event) => { const remove = event.target.closest("[data-room-remove]"); if (!remove || $("#room-player-inputs").children.length <= 2) return; remove.closest(".room-player-row").remove(); renumberRoomPlayers(); clearRoomPlayerError(); });
...
document.addEventListener("submit", async (event) => { if (event.target.id !== "room-player-form") return; event.preventDefault(); const form = event.target, names = [...form.querySelectorAll("#room-player-inputs input")].map((input) => input.value.trim()), error = $("#room-player-error"), submit = form.querySelector('button[type="submit"]'); clearRoomPlayerError(); if (names.length < 2) { error.textContent = "Lägg till minst två spelare."; error.hidden = false; return; } if (names.some((name) => !name)) { error.textContent = "Ange namn för alla deltagare."; error.hidden = false; return; } if (new Set(names.map((name) => name.toLowerCase())).size !== names.length) { error.textContent = "Alla deltagare behöver olika namn."; error.hidden = false; return; } submit.disabled = true; try { await createLocalMatch("room", names); $("#app-dialog").hidden = true; $("#dialog-confirm").hidden = false; } catch (failure) { error.textContent = failure.message || "Matchen kunde inte startas. Försök igen."; error.hidden = false; } finally { submit.disabled = false; } });

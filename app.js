...
document.addEventListener("click", (event) => { const button = event.target.closest("[data-friend-career]"); if (!button) return; const ownAvatar = { ...(state.avatar || {}) }; state.avatarDirty = true; state.friendCareerOpen[button.dataset.friendCareer] = !state.friendCareerOpen[button.dataset.friendCareer]; save(); renderFriends(); state.avatar = ownAvatar; renderAvatar(); });

...
async function syncFriends() {
  ...
  supabaseAuth.dataRequest("rpc/digihits_my_avatar", {}, "POST").then((rows) => { const saved = rows?.[0], hasLocalChoice = avatarStyles.includes(state.avatar?.genre) && Number.isInteger(Number(state.avatar?.variant)) && Number(state.avatar.variant) >= 0 && Number(state.avatar.variant) < 6; if (!saved || state.avatarDirty || hasLocalChoice) return; state.avatar = { ...state.avatar, genre: saved.avatar_genre, variant: Number(saved.avatar_variant), traits: saved.avatar_traits || state.avatar?.traits }; save(); renderAvatar(); }).catch(() => {});
}

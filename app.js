...
function renderFriends() {
  ...
  friends.querySelectorAll(".friend-row").forEach((row) => { const name = row.querySelector(":scope > strong"); if (!name || !row.querySelector(".friend-stats")) return; const avatar = document.createElement("span"), choice = avatarChoice({ genre: row.dataset.avatarGenre, variant: Number(row.dataset.avatarVariant) }); avatar.className = "mini-avatar friend-avatar avatar-art"; avatar.style.cssText = avatarArtStyle(choice.genre, choice.variant); name.before(avatar); name.classList.add("friend-name-with-avatar"); });
}

...
function render() {
  ...
  const renderCard = (match) => {
    const room = localMatch(match)?.mode === "room", solo = isSoloMatch(match) && !room;
    const soloScore = solo ? soloProgress(match) : null;
    const label = room ? "ÖPPNA RUMSMATCH HÄR" : solo ? "ÖPPNA SOLOMATCH HÄR" : match.status === "active" ? "ÖPPNA MATCH HÄR" : "VISA MATCHÖVERSIKT HÄR";

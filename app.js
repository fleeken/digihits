...
function decorateLocalMatch(match) {
  ...
  match.players = local.players.map((player, index) => ({ id: `${match.code}-${index}`, user_id: local.mode === "room" ? `room-${index}` : index === local.current ? state.userId : `local-${index}`, display_name: player.name, avatar_genre: player.avatar?.genre, avatar_variant: player.avatar?.variant, turn_order: index, rounds_started: player.rounds || 0, locked_timeline: player.timeline || [], last_round: player.lastRound || null, swap_cards: 0 }));
  if (local.mode === "room") match.currentUserId = `room-${local.current}`;
  return match;
}
...
function roomPlayerRow(number) { return `<div class="room-player-row"><span class="room-player-number">${number}.</span><input maxlength="18" placeholder="Spelare ${number}" aria-label="Spelare ${number}"><button type="button" data-room-up aria-label="Flytta upp">↑</button><button type="button" data-room-down aria-label="Flytta ner">↓</button><button type="button" data-room-remove aria-label="Ta bort deltagare">🗑</button></div>`; }
function renumberRoomPlayers() { [...$("#room-player-inputs").children].forEach((row, index) => { row.querySelector(".room-player-number").textContent = `${index + 1}.`; const input = row.querySelector("input"); input.setAttribute("aria-label", `Spelare ${index + 1}`); if (!input.value) input.placeholder = `Spelare ${index + 1}`; }); }
function showRoomAvatarPicker(room, index) {
  const player = room.players[index], avatar = avatarChoice({ display_name: player.name, ...player.avatar });
  $("#dialog-title").textContent = `Avatar för ${player.name}`;
  $("#dialog-message").innerHTML = `<div class="room-avatar-picker"><div class="avatar-genre-grid">${avatarStyles.map((genre) => `<button type="button" data-room-avatar-genre="${genre}" class="${genre === avatar.genre ? "is-selected" : ""}">${genre}</button>`).join("")}</div><div class="avatar-variant-grid">${Array.from({ length: 6 }, (_, variant) => `<button type="button" class="avatar-art ${variant === avatar.variant ? "is-selected" : ""}" style="${avatarArtStyle(avatar.genre, variant)}" data-room-avatar-variant="${variant}" aria-label="Avatar ${variant + 1}"></button>`).join("")}</div></div>`;
  $("#dialog-cancel").hidden = true; $("#dialog-confirm").hidden = false; $("#dialog-confirm").textContent = "KLAR";
  $("#dialog-confirm").onclick = () => { $("#app-dialog").hidden = true; }; $("#app-dialog").hidden = false;
...
document.addEventListener("click", (event) => {
  ...
  const room = localMatch(), index = Number($("#dialog-message").dataset.roomAvatarIndex);
  if (!room || !room.players[index]) return;
  const selected = avatarChoice({ display_name: room.players[index].name, ...room.players[index].avatar });
  room.players[index].avatar = { genre: genre?.dataset.roomAvatarGenre || selected.genre, variant: variant ? Number(variant.dataset.roomAvatarVariant) : selected.variant };
  save(); decorateLocalMatch(state.matches.find((match) => match.code === state.activeMatchCode)); renderRoundPlayers(); showRoomAvatarPicker(room, index);
});
...
artistAliases[answerText("Carola")] = ["Carola Häggkvist"];
...
function render() {
  ...
  const achievements = [
    ...
    ["soloDaily", "★", "Solisten", "Slutför en omgång i en solomatch.", "solo"],
    ["socialToneDaily", "♥", "Social ton", "Slutför en omgång i en onlinematch.", "online"],
    ...
    ["eveningDj", "♫", "Kvällens DJ", "Slutför omgångar mot tre olika personer.", "online"],
    ["fullSpeed", "↯", "Full fart", "Slutför minst fem omgångar i solomatch och minst fem omgångar i onlinematch.", "solo OCH online"],
  ...
  const renderCard = (match) => {
    ...
    return `<article class="match ${room ? "room" : solo ? "solo" : match.status}">${solo ? "" : `<div class="match-status match-status-top">● ${escapeHtml(status)}</div>${unread ? `<button class="match-chat-alert" data-open-chat="${match.code}" title="Nya chattmeddelanden" aria-label="Öppna chatt" type="button">✉<b>${unread}</b></button>` : ""}${room ? "" : `<button class="match-icon delete-icon match-delete-top" data-delete-match="${match.code}" title="Lämna match" aria-label="Lämna match" type="button">🗑</button>`}`}${solo ? `<div class="match-top"><strong>Solomatch</strong></div><div class="solo-card-stats"><div><strong>${soloScore.correct}/10</strong><small>RÄTT PLACERADE</small></div><div><strong>${soloScore.mistakes}</strong><small>FELPLACERADE</small></div><div><strong>${soloRounds}</strong><small>${soloRounds === 1 ? "OMGÅNG" : "OMGÅNGAR"}</small></div></div>` : `<div class="match-player-list">${playerRows}</div>`}<div class="match-footer"><button class="match-open" data-open-match="${match.code}" type="button">${label}</button>${solo || room ? `<div class="match-card-actions"><button class="match-icon delete-icon" data-delete-match="${match.code}" title="Lämna match" aria-label="Lämna match" type="button">🗑</button></div>` : ""}</div>${solo || room ? "" : `<div class="match-code match-code-bottom">MATCHKOD <strong>${match.code}</strong></div>`}</article>`;
  };
...
async function createRandomOnlineMatch() {
  ...
  try {
    ...
    rememberTrack(starter); state.changeTrackCards = 0; state.career.createdMatchCodes.push(matchCode); save(); await Promise.all([syncMatches(), syncFriends()]); openMatch(matchCode); dialog(`Matchen är skapad och ${invited?.recipient_name || "en slumpvald person"} har bjudits in.`);
  } catch (error) {
...
document.addEventListener("click", (event) => { if (!event.target.closest("#add-room-player")) return; const holder = $("#room-player-inputs"); if (holder.children.length < 8) holder.insertAdjacentHTML("beforeend", roomPlayerRow(holder.children.length + 1)); });
document.addEventListener("click", (event) => { const remove = event.target.closest("[data-room-remove]"); if (!remove) return; remove.closest(".room-player-row").remove(); renumberRoomPlayers(); });
document.addEventListener("click", (event) => { const up = event.target.closest("[data-room-up]"), down = event.target.closest("[data-room-down]"); if (!up && !down) return; const row = (up || down).closest(".room-player-row"); if (up && row.previousElementSibling) row.parentElement.insertBefore(row, row.previousElementSibling); if (down && row.nextElementSibling) row.parentElement.insertBefore(row.nextElementSibling, row); renumberRoomPlayers(); });
document.addEventListener("submit", async (event) => { if (event.target.id !== "room-player-form") return; event.preventDefault(); const names = [...event.target.querySelectorAll("input")].map((input) => input.value.trim()), error = $("#room-player-error"); if (names.length < 2 || names.some((name) => !name)) { error.textContent = "Ange namn för alla deltagare (minst två)."; error.hidden = false; return; } if (new Set(names.map((name) => name.toLowerCase())).size !== names.length) { error.textContent = "Alla deltagare behöver olika namn."; error.hidden = false; return; } $("#app-dialog").hidden = true; $("#dialog-confirm").hidden = false; try { await createLocalMatch("room", names); } catch (failure) { alert(failure.message); } });
...
$("#friend-search-form")?.addEventListener("submit", async (event) => { event.preventDefault(); const input = $("#friend-search"), feedback = $("#friend-feedback"), result = $("#friend-search-result"), requested = input.value.trim(); if (!requested) return; result.hidden = true; result.replaceChildren(); feedback.hidden = true; try { const rows = await supabaseAuth.dataRequest("rpc/digihits_find_friend", { requested }, "POST"), player = rows?.[0]; if (!player) throw new Error("not-found"); const avatar = avatarChoice(player), existing = state.friends.some((friend) => String(friend.friend_id) === String(player.user_id)), pending = state.sentFriendRequests.some((request) => String(request.recipient_id) === String(player.user_id) && request.status === "pending"); result.innerHTML = `<article class="friend-search-hit ${existing ? "is-friend" : pending ? "is-pending" : ""}"><span class="mini-avatar friend-avatar avatar-art" style="${avatarArtStyle(avatar.genre, avatar.variant)}"></span><strong>${escapeHtml(player.display_name)}</strong>${existing ? `<span class="already-friend">REDAN VÄN MED</span>` : pending ? `<span class="already-friend">VÄNFÖRFRÅGAN SKICKAD</span>` : `<button class="button button-green" type="button" data-add-searched-friend="${escapeHtml(player.display_name)}">LÄGG TILL VÄN</button>`}</article>`; result.hidden = false; } catch { feedback.textContent = "Ingen spelare hittades. Kontrollera spelarnamnet."; feedback.classList.add("error"); feedback.hidden = false; } });
...
$("#lock-placement").addEventListener("click", async () => {
  ...
  const activeMatch = state.matches.find((match) => match.code === state.activeMatchCode), room = localMatch(activeMatch)?.mode === "room", solo = isSoloMatch(activeMatch) && !localMatch(activeMatch);
  ...
  if (!room) { daily.rounds += 1; daily.solo ||= solo; daily.online ||= !solo; daily.soloRounds = Number(daily.soloRounds || 0) + (solo ? 1 : 0); daily.onlineRounds = Number(daily.onlineRounds || 0) + (solo ? 0 : 1);
  ...
  save();
  if (!room && currentPlacementCorrect && state.roundUnlocked.length + 1 >= 3) grantDailyAchievement("hattrick", "Hattrick");
  ...
  if (!room && currentPlacementCorrect && hasCorrectSongGuess(resultCard) && state.changeTrackCards < 3) { if (solo && state.changeTrackCards >= 2) grantDailyAchievement("triple", "Trippel"); state.pendingSwapAward = { matchCode: state.activeMatchCode, cardId: resultCard.id }; state.swapUsedThisRound = false; save(); earnedSwapCard = true; }
  ...
  let soloOutcome;
  if (!currentPlacementCorrect || solo) { try { soloOutcome = await handoverTurn(currentPlacementCorrect ? null : resultSnapshot.timeline); if (room && !soloOutcome?.won) $("#wrong-overview").dataset.roomHandoverName = soloOutcome.nextPlayerName; } catch (error) { alert(error.message); return; } }
  if (soloOutcome?.won && solo) { grantDailyAchievement("soloWin", "Solovinst"); if (Number(soloOutcome.soloSummary?.mistakes || 0) === 0) grantDailyAchievement("soloFlawless", "Felfri"); state.pendingResult = null; delete state.roundResumeViews[state.activeMatchCode]; save(); finishAchievementAwards(); }
  ...
  else if (earnedSwapCard && !room) dialog(solo ? "Grattis, du vann ett byt-låt-kort eftersom du gissade rätt för både artist och låtnamn! Byt-låt-kort påverkar inte antalet genomförda omgångar." : "Grattis, du vann ett byt-låt-kort eftersom du gissade rätt för både artist och låtnamn!");
  else if (!room && currentPlacementCorrect && hasCorrectSongGuess(resultCard) && state.changeTrackCards >= 3) dialog("Du gissade rätt för både artist och låtnamn, men du har redan 3/3 byt-låt-kort.");
...
$(".brand small").textContent = `v${APP_VERSION}`;
render();

...
function renderFriends() {
  const requests = $("#friend-requests"), sent = $("#friend-sent-requests"), friends = $("#friends-list"), invites = $("#friend-invites"); if (!requests || !sent || !friends || !invites) return;
  const unreadCount = Object.values(state.friendChatUnread).reduce((total, count) => total + Number(count || 0), 0), requestCount = state.friendRequests.length; $("#friend-request-count").textContent = `Vänförfrågan ${requestCount}`; $("#friend-request-count").hidden = !requestCount; $("#friend-chat-alert").hidden = !unreadCount; $("#friend-chat-count").textContent = String(unreadCount);
  ...
  sent.innerHTML = state.sentFriendRequests.length ? `<h3 class="friend-section-title">Skickade vänförfrågningar</h3>${state.sentFriendRequests.map((request) => request.status === "accepted" ? `<article class="friend-row friend-request-accepted"><strong>Du är nu vän med ${escapeHtml(request.display_name)}.</strong><button class="button button-secondary" data-dismiss-friend-request="${request.request_id}" type="button">OK</button></article>` : request.status === "declined" ? `<article class="friend-row friend-request-declined"><strong>${escapeHtml(request.display_name)} avvisade din vänförfrågan.</strong><button class="button button-secondary" data-dismiss-friend-request="${request.request_id}" type="button">OK</button></article>` : `<article class="friend-row"><strong>Vänförfrågan skickad till ${escapeHtml(request.display_name)}, väntar på svar.</strong></article>`).join("")}` : "";
...
async function syncFriends() {
  ...
  const [friends, requests, invites, unreads, sentRequests] = await Promise.all([supabaseAuth.dataRequest("rpc/digihits_my_friends", {}, "POST"), supabaseAuth.dataRequest("rpc/digihits_my_friend_requests", {}, "POST"), supabaseAuth.dataRequest("rpc/digihits_my_match_invites", {}, "POST"), supabaseAuth.dataRequest("rpc/digihits_my_friend_unreads", {}, "POST"), supabaseAuth.dataRequest("rpc/digihits_my_sent_friend_requests", {}, "POST")]);
  state.friends = friends || []; state.friendRequests = requests || []; state.friendInvites = invites || []; state.friendChatUnread = Object.fromEntries((unreads || []).map((item) => [item.friend_id, Number(item.unread_count)])); state.sentFriendRequests = sentRequests || []; save(); renderFriends();
}
function render() {
  updateTurnBadge();
...
async function loadOverviewPlayers(matchId, isYourTurn, solo = false) {
  try {
    const players = await supabaseAuth.dataRequest(`online_players?match_id=eq.${matchId}&active=eq.true&select=id,user_id,display_name,turn_order,locked_timeline,last_round,rounds_started&order=turn_order`);
    const match = state.matches.find((item) => item.id === matchId), friendBox = $("#match-friend-invites");
    if (friendBox && match && !solo && !match.locked) { const playerNames = new Set(players.map((player) => String(player.display_name).toLocaleLowerCase("sv-SE"))); const inviteableFriends = state.friends.filter((friend) => String(friend.friend_id) !== String(state.userId) && !playerNames.has(String(friend.display_name).toLocaleLowerCase("sv-SE"))); friendBox.hidden = false; friendBox.innerHTML = inviteableFriends.length ? `<small>BJUD IN VÄN TILL MATCHEN</small><div>${inviteableFriends.map((friend) => `<button class="button button-secondary" data-invite-friend="${friend.friend_id}" type="button">${escapeHtml(friend.display_name)} · BJUD IN</button>`).join("")}</div>` : `<small>DU HAR INGA FLER VÄNNER ATT BJUD IN TILL DENNA MATCH.</small>`; }
...
$("#friends-section")?.addEventListener("click", async (event) => {
  const answer = event.target.closest("[data-friend-answer]"), dismiss = event.target.closest("[data-dismiss-friend-request]"), create = event.target.closest("[data-create-friend-match]"), remove = event.target.closest("[data-remove-friend]"), invite = event.target.closest("[data-join-friend-match]"), chat = event.target.closest("[data-open-friend-chat]");
  try {

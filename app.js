...
async function animateCardDeal(card, starter = null) {
  ...
  const fly = async (flyingCard, secret, target, duration) => {
    ...
    const startX = deckBounds.left + deckBounds.width / 2 - targetBounds.width / 2, startY = deckBounds.top - targetBounds.height * .24, exitX = deckBounds.left + deckBounds.width / 2 - targetBounds.width / 2, exitY = deckBounds.top - targetBounds.height * .88;
    ...
    const lengths = points.slice(1).map((point, index) => Math.hypot(point.x - points[index].x, point.y - points[index].y)), totalLength = lengths.reduce((sum, length) => sum + length, 0), offsets = [0, .32, ...curve.map((_, index) => .32 + .68 * ((index + 1) / curve.length))];
    const travelDuration = Math.max(duration, Math.min(3300, totalLength / .24));
    const frames = points.map((point, index) => { const t = point.t ?? (index ? 0 : 0), scale = index === 0 ? .3 : index === 1 ? 1 : 1 + Math.sin(Math.PI * t) * .035, rotation = index < 2 ? -5 + index * 3 : Math.sin(Math.PI * t * 2) * 2.2; return { offset: offsets[index], transform: `translate3d(${point.x}px,${point.y}px,0) rotate(${rotation}deg) scale(${scale})`, opacity: 1 }; });
...
function renderRoundPlayers() {
  ...
  document.querySelectorAll(".round-player-strip").forEach((strip) => {
    ...
    strip.hidden = false;
    strip.innerHTML = `<p>Nu spelar: <strong>${escapeHtml(state.playerName)}</strong> <span>▶</span></p><div>${players.map((player) => { const mine = String(player.user_id) === String(state.userId), score = Math.max(1, Number(player.last_round?.score?.correct) || (player.locked_timeline || []).length || 1), initial = escapeHtml(String(player.display_name || "?").trim().charAt(0).toUpperCase()); return `<article class="${mine ? "is-current" : ""}"><i>${initial}</i><span>${escapeHtml(player.display_name || "Spelare")}<b>${score}</b></span>${mine ? "<small>DIN TUR</small>" : ""}</article>`; }).join("")}</div>`;
  });
}
...
function renderRoundResult(correct, card = activeCard(), snapshot = null) {
  ...
  $("#result-timeline").classList.remove("timeline-centered"); $("#result-timeline").innerHTML = timeline.map((item) => `<article class="year-card ${item.status === "STARTKORT" || item.status === "LÅST" ? "locked-card" : /FEL ?PLACERAT/.test(item.status) ? "misplaced-card" : solo ? "correct-card" : "unlocked-card"}"${item.status === "STARTKORT" ? " style=\"border-color:#58657a;background:#202632\"" : ""}><strong>${item.year}</strong><small><span class="card-song">${item.title}<br>${item.artist}</span><span class="card-status">${cardStatusLabel(item.status)}</span></small></article>`).join("");
...
if ("serviceWorker" in navigator) navigator.serviceWorker.register("sw.js?v=5.79", { updateViaCache: "none" }).then((registration) => registration.update()).catch(() => {});

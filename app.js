...
document.addEventListener("click", (event) => { const button = event.target.closest("[data-round-player]"); if (!button) return; const round = latestRounds[button.dataset.roundPlayer]; if (!round) { dialog("Spelaren har ingen spelad omgång ännu."); return; } latestRoundReturnView = currentView; showLatestRound(round); });
...
if ("serviceWorker" in navigator) navigator.serviceWorker.register("sw.js?v=5.92", { updateViaCache: "none" }).then((registration) => registration.update()).catch(() => {});

...
function showRoomSetup() {
  ...
  $("#dialog-message").innerHTML = `<button class="dialog-back-step" data-match-back type="button">← TILLBAKA</button><form id="room-player-form" class="room-player-form"><p>Lägg till deltagarna i den ordning mobilen ska skickas runt. Ändra turordningen med pilarna.</p><div id="room-player-inputs"><div class="room-player-row"><input maxlength="18" value="${escapeHtml(state.playerName)}" aria-label="Spelare 1"><button type="button" data-room-up aria-label="Flytta upp">↑</button><button type="button" data-room-down aria-label="Flytta ner">↓</button></div><div class="room-player-row"><input maxlength="18" placeholder="Spelare 2" aria-label="Spelare 2"><button type="button" data-room-up aria-label="Flytta upp">↑</button><button type="button" data-room-down aria-label="Flytta ner">↓</button></div></div><button class="button button-secondary" id="add-room-player" type="button">+ LÄGG TILL SPELARE</button><button class="button button-green" type="submit">STARTA MATCH</button><small id="room-player-error" class="friend-feedback error" hidden></small></form>`;
...
function render() {
  ...
  const renderCard = (match) => {
    ...
    return `<article class="match ${room ? "room" : solo ? "solo" : match.status}">${solo || room ? "" : `<div class="match-status match-status-top">● ${status}</div>${unread ? `<button class="match-chat-alert" data-open-chat="${match.code}" title="Nya chattmeddelanden" aria-label="Öppna chatt" type="button">✉<b>${unread}</b></button>` : ""}<button class="match-icon delete-icon match-delete-top" data-delete-match="${match.code}" title="Lämna match" aria-label="Lämna match" type="button">🗑</button>`}${solo ? `<div class="match-top"><strong>Solomatch</strong></div><div class="solo-card-stats"><div><strong>${soloScore.correct}/10</strong><small>RÄTT PLACERADE</small></div><div><strong>${soloScore.mistakes}</strong><small>FELPLACERADE</small></div><div><strong>${soloRounds}</strong><small>${soloRounds === 1 ? "OMGÅNG" : "OMGÅNGAR"}</small></div></div>` : `<div class="match-player-list">${playerRows}</div>`}<div class="match-footer"><button class="match-open" data-open-match="${match.code}" type="button">${label}</button>${solo || room ? `<div class="match-card-actions"><button class="match-icon delete-icon" data-delete-match="${match.code}" title="Lämna match" aria-label="Lämna match" type="button">🗑</button></div>` : ""}</div>${solo || room ? "" : `<div class="match-code match-code-bottom">MATCHKOD <strong>${match.code}</strong></div>`}</article>`;
  };
...
document.addEventListener("click", (event) => { if (!event.target.closest("#add-room-player")) return; const holder = $("#room-player-inputs"), count = holder.children.length; if (count >= 8) return; holder.insertAdjacentHTML("beforeend", `<div class="room-player-row"><input maxlength="18" placeholder="Spelare ${count + 1}" aria-label="Spelare ${count + 1}"><button type="button" data-room-up aria-label="Flytta upp">↑</button><button type="button" data-room-down aria-label="Flytta ner">↓</button></div>`); });
document.addEventListener("click", (event) => { const up = event.target.closest("[data-room-up]"), down = event.target.closest("[data-room-down]"); if (!up && !down) return; const row = (up || down).closest(".room-player-row"); if (up && row.previousElementSibling) row.parentElement.insertBefore(row, row.previousElementSibling); if (down && row.nextElementSibling) row.parentElement.insertBefore(row.nextElementSibling, row); [...row.parentElement.children].forEach((item, index) => { const input = item.querySelector("input"); input.setAttribute("aria-label", `Spelare ${index + 1}`); if (!input.value) input.placeholder = `Spelare ${index + 1}`; }); });

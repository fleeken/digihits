...
async function syncFriends() {
  ...
  supabaseAuth.dataRequest("rpc/digihits_my_avatar", {}, "POST").then((rows) => { const saved = rows?.[0]; if (!saved || state.avatarDirty) return; state.avatar = { ...state.avatar, genre: saved.avatar_genre, variant: Number(saved.avatar_variant), traits: saved.avatar_traits || state.avatar?.traits }; save(); renderAvatar(); }).catch(() => {});
}

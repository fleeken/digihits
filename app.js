...
function startCurrentTrack() { clearInterval(songTimer); loadedSpotifyCardId = null; songPosition = 0; songDuration = 0; $("#song-timeline").hidden = true; wasPausedByUser = false; pausedForNavigation = false; setPlayButton(false); spotifyPlayer?.pause().catch(() => {}); if (!mobileBrowser && supabaseAuth.spotify()) { trackStartPromise = playCurrentTrack().catch(() => { setPlayButton(false); }).finally(() => { trackStartPromise = null; }); } }
